...
import { useConvexMutation } from "@/hooks/useConvexHook";
import { api } from "@/convex/_generated/api";
import { toast } from "sonner";

export default function ProjectCard({ project, onEdit }) {
  const { mutate: deleteProject, loading } = useConvexMutation(
    api.projects.deleteProject
  );

  if (!project) {
    console.warn("ProjectCard: No project provided");
    return null;
  }
  ...
  let lastUpdated;
  try {
    const updateDate = project.updatedAt || project.createdAt || Date.now();
    lastUpdated = formatDistanceToNow(new Date(updateDate), {
      addSuffix: true,
    });
  } catch (error) {
